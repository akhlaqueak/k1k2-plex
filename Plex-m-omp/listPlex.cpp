...
namespace ListPlex{
    ...
    Graph buildSubgraph(const Graph &g,const vector<int>& blk,const vector<int>& left,const vector<uint8_t> &inBlk, const vector<uint8_t> &inLeft,const int hopSz,vector<uint8_t> &isNei,vector<uint8_t> &commonMtx){  
        ...
        for (int i = 0; i < blkSz; i++){
            ...
            for (int j = 0; j < blkSz; j++){
                ...
                if(j==hopSz-1)vertices[i].degreeHop=cnt;//顶点i的邻居且是lead顶点的1-hop的数量
            }
            //add left edges
            cnt = 0;
            ...
            if(i<hopSz)edgesHop+=vertices[i].degreeHop;//顶点i是lead顶点的邻居或本身,edgesHop: lead顶点的1-hop+本身构成的图中的边数*2
    ...
    Graph peelGraph(const Graph &g,bool* const mark,int * const resNei){
        ...
        for(int i=0;i<pn;++i)map[leadList.A[i]]=i; // old2new
        ...
        for (int i = 0; i < pn; i++){
            ...
            for(int j=0;j<g.V[ori].degree;++j){
                ...
                if(mark[nei]) vertices[i].Neighbors[cursor++]=map[nei];//把新顶点号加入其中
            }
        }
        delete[] map;
        leadList.del();
        ...
        for(int i=0;i<hopSz;++i){
            for(int j=0;j<i;++j){
                const int common=utils::commonEle(
                    ...
                    subg.V[j].Neighbors,
                    subg.V[j].degreeHop
                );//i,j两个lead顶点的1-hop顶点 在lead顶点的1-hop中的交集大小
    ...
    void decomposableSearch(const Graph &g){
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp for
            for(int i=0;i<pn;++i) mark[i]=true;
            ...
            for(int idx = 0; idx < pn; idx++){
                ...
                if (blkSz>=lb) {
                    ...
                    for (int j = 0; j < subg.n; j++){
                        ...
                        if(j<hopSz)cand1.add(j);//1-hop和本身
                        else cand2.add(j);//2-hop
                    }
