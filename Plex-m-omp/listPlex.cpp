...
namespace ListPlex
{
    ...
    {
        const graph<int> &subg; // add & reference??
        RandSet<int> uni;

        DecomposeMaxChecker(const graph<int> &_subg)
            : subg(_subg), uni(CAP_LIMIT2) {}
        // 能否加入现在的plex形成更大的plex
        ...
        bool isMaximal(int *list, int szplex, int *neiInP)
        {
            ...
            if (nsat)
            {
                ...
                {
                    int u = commons[i];
                    if (canGlobalAdd(u, szplex, list))
                    { // not maximal because we can add u to the solution
                        max = false;
                        break;
                    }
                }
    ...
    int decompose(const Graph &g, const int vstart, vector<int> &neibors_1a2hop, vector<int> &neibors_1a2hopLeft, const int *dpos, vector<uint8_t> &visited, vector<int> &count, vector<int> &neibors, vector<int> &neiborsLeft)
    {
        ...
        // origin中元素的邻居与1hop邻居的交集大小 pruing seed graph
        ...
        // the number of common neighbors with vstart >= lb+1-2*k，对order左边的点能否加入形成更大的plex应该+1
    ...
    CLEAN:
        ...
        neibors.clear();
        neiborsLeft.clear();
        return sz + 1; // 返回的right中的1hop+本身.
    }
    // 构建剪枝后的种子图，以及与序小于种子点部分的二部图(neighbor_left)
    Graph buildSubgraph(const Graph &g, const vector<int> &blk, const vector<int> &left, const vector<uint8_t> &inBlk, const vector<uint8_t> &inLeft, const int hopSz, vector<uint8_t> &isNei, vector<uint8_t> &commonMtx)
    {
        ...
        {
            const int origin = blk[i]; // 原编号origin 重新编号为i
            int ne = 0, neLeft = 0;
            ...
            vertices[i].degree = ne;         // 在大于的ordered部分
            vertices[i].degreeLeft = neLeft; // 在小于的ordered部分
            ...
            {
                const int nei = blk[j];
                commonMtx[i * blkSz + j] = isNei[nei]; // 是顶点i的邻居
                if (isNei[nei])
                {
                ...
                if (j == hopSz - 1)
                    vertices[i].degreeHop = cnt; // 顶点i的邻居且是lead顶点的1-hop的数量
            }
            // add left edges
            cnt = 0;
            ...
            if (i < hopSz)
                edgesHop += vertices[i].degreeHop; // 顶点i是lead顶点的邻居或本身,edgesHop: lead顶点的1-hop+本身构成的图中的边数*2
            edges += vertices[i].degree;           // 子图中的边数*2
        }
        const bool proper = (double(edgesHop) / (hopSz * (hopSz - 1))) < 0.9 && (double(edges) / (blkSz * (blkSz - 1))) < 0.85; // 是否稀疏？
    ...
    Graph peelGraph(const Graph &g, bool *const mark, int *const resNei)
    {
        ...
        for (int i = 0; i < pn; ++i)
            map[leadList.A[i]] = i; // old2new
        vertex<int> *vertices = newA(vertex<int>, pn);
        for (int i = 0; i < pn; i++)
        {
            const int ori = leadList.A[i]; // 原顶点号
            const int en = resNei[ori];    // peeling后的度数
            ...
            for (int j = 0; j < g.V[ori].degree; ++j)
            {
                ...
                if (mark[nei])
                    vertices[i].Neighbors[cursor++] = map[nei]; // 把新顶点号加入其中
            }
        }
        delete[] map;
        leadList.del();
        ...
        for (int i = 0; i < hopSz; ++i)
        {
            for (int j = 0; j < i; ++j)
            {
                const int common = utils::commonEle(
                    ...
                    subg.V[i].degreeHop,
                    subg.V[j].Neighbors,
                    subg.V[j].degreeHop); // i,j两个lead顶点的1-hop顶点 在lead顶点的1-hop中的交集大小

                if (commonMtx[i * blkSz + j])
                {
                    if (common > thresCC1)
                        commonMtx[i * blkSz + j] = LINK2MORE;
                    else if (common == thresCC1)
                        commonMtx[i * blkSz + j] = LINK2EQUAL;
                    else
                        commonMtx[i * blkSz + j] = LINK2LESS;
                }
                else
                {
                    if (common > thresCC2)
                        commonMtx[i * blkSz + j] = UNLINK2MORE;
                    else if (common == thresCC2)
                        commonMtx[i * blkSz + j] = UNLINK2EQUAL;
                    else
                        commonMtx[i * blkSz + j] = UNLINK2LESS;
                }
    ...
    void decomposableSearch(const Graph &g)
    {
        ...
        for (int i = 0; i < g.n; ++i)
            dpos[i] = INT_MAX;
        int validblk = 0;
        size_t cntMaxPlex = 0, ttlTaskCnt = 0;
        ...
        // degeneracy order得到dpos[u](节点u所在的位置)，dseq[i](第i个位置的节点)
        ListLinearHeap<int> *linear_heap = new ListLinearHeap<int>(pn, pn - 1); // unique_ptr is better
...
#pragma omp parallel
        {
            ...
            thread_local vector<int> neiInG(CAP_LIMIT2); // 在P\cup Cand中的degree
...
#pragma omp for schedule(dynamic)
            for (int idx = 0; idx < pn; idx++)
            {
                ...
                left.clear();
                // pruning seed graph
                const int hopSz = decompose(peelG, vstart, blk, left, dpos, visited, count, neibors, neiborsLeft); // 1-hop加本身
                const int blkSz = blk.size();                                                                      // 1-hop+self+2-hop;
                if (blkSz >= lb)
                {
                    const int hop2Sz = blkSz - hopSz; // 2-hop size
                    const int leftSz = left.size();
                    validblk++; // 有效的seed graph
                    ...
                    // pruning seed set
                    if (subg.proper)
                        buildCommonMtx(subg, blkSz, hopSz, commonMtx);
                    // build search context
                    plex.resize(hopSz + k); // vi_lead_plex至多包括所有1-hop加上k个non-neighbor
                    cand1.resize(hopSz);
                    cand2.resize(hop2Sz);
                    excl.resize(blkSz);
                    exclBK.resize(blkSz);
                    ...
                    {
                        neiInG[j] = subg.V[j].degreeHop; // 此时P为空，Cand为1-hop和本身
                        if (j < hopSz)
                            cand1.add(j); // 1-hop和本身
                        else
                            cand2.add(j); // 2-hop
                    }
...
#pragma omp parallel reduction(+ : cntMaxPlex) reduction(+ : ttlTaskCnt)
        {
            cntMaxPlex += cntT;
            ttlTaskCnt += taskCnt;
        }
...
int main(int argc, char **argv)
{
    if (argc == 4)
    {
        ...
        lb = atoi(argv[3]);
        bd = lb - k;
        // graph:(vertex,n,m,allocated)
        // vertex:neighbor,degree
        // allocated(2+n+m): allocated[0]=n,allocated[1]=m,offset,edges
    ...
    {
        fprintf(stderr, "usage: listPlex <filename> <k> <lb>\n");
    }
    return 0;
}