...
void EnuBundle::multiRecurSearch(vector<ui> &doing, ui szmax)
{
	...
	{ // szmax +1 branches
		...
		{ // idx== szmax, the last branch
			vector<ui> doreduced;
			for (ui j = idx; j < doing.size(); j++)
			{
				if (Cand.contains(doing[j]))
				{ // not reduced, we need to recover
...
ui EnuBundle::dbgCheckSolution()
{
	...
	for (ui u = 0; u < n; u++)
	{
		if (isInBlock(u) && P.contains(nID[u]))
			continue; // in subgraph
...
void EnuBundle::enumPlex(ui _k, ui _lb, uli _maxsec, ui _isdecompose, ui _quiete)
{
	...
	if (decompose)
	{													// decompose the graph and enumerate k-plexes in smaller subgraphs
